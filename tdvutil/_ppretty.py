...
from typing import List, Optional, Dict, Type, Tuple, Set, Union
...
def ppretty(obj: object, *, indent: str = '    ', depth: int = 4,
    ...
    seq_brackets: Dict[type, Tuple[str, str]] = {
        list: ('[', ']'), tuple: ('(', ')'), set: ('set([', '])'), dict: ('{', '}')}
    seq_types = tuple(seq_brackets.keys())

    ignore = set(ignore) if ignore else set()

    def inspect_object(current_obj: object, current_depth: int,
                       current_width: int,
                       seq_type_descendant: bool = False) -> List[str]:
